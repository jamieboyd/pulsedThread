...
/* constants for timing methods:
    ACC_MODE_SLEEPS                 - thread sleeps for duration of period
    ACC_MODE_SLEEPS_AND_SPINS       - thread sleeps for period - kSLEEPTURNAROUND microseconds, then spins for remaining time
    ACC_MODE_SLEEPS_AND_OR_SPINS    - sleep time is re-calculated for each duration, sleep is countermanded if thread is running late
    kSLEEPTURNAROUND                -how long, in microseconds, we aim to spin for the end of pulse timing in accuracy levels 1 and 2
 */
 
 // define beVerbose to non-zero to print out messages that mey be useful for debugging or understanding how the code runs 
 #define beVerbose 1
 
const int ACC_MODE_SLEEPS = 0;
const int ACC_MODE_SLEEPS_AND_SPINS =1;
const int ACC_MODE_SLEEPS_AND_OR_SPINS =2;
const int kSLEEPTURNAROUND= 200;
...
struct taskParams {
	...
	int volatile doTask; // incremented when tasks are requested, decremented when tasks are done
	// raw pulse durations and number of pulses, in microseconds
	unsigned int volatile pulseDelayUsecs; // duration of low time in microseconds, can be 0, in which case loFunc is never called for a train or infinite train
	unsigned int volatile pulseDurUsecs; // duration of high time in microseconds, must be > 0
	unsigned int volatile nPulses; // number of pulses in a train, 0 for infinite train, 1 for a single pulse
	// requested train durations, train fequencies, and train duty cycles. Same information as above
	float volatile trainDuration; // duration of train, in seconds, or 0 for infinite train
	float volatile trainFrequency; // frequency in Hz, i.e., pulses/second
	float volatile trainDutyCycle; // pulseDurUsecs/(pulseDurUsecs + pulseDelayUsecs)
	void *  volatile taskCustomData; // pointer to custom taskData
	void (*loFunc)(void * volatile) ; // function to run for low part of pulse, gets pointer to taskData
	void (*hiFunc)(void * volatile );// function to run for high part of pulse, gets pointer to taskData
...
/***************************************************************************************
Utility functions for converting between time-based info (trainDuration, frequency, dutyCycle) and 
pulse -based info (pulseDelay, pulseDuation, number of pulses) 
In the header file as you might want to use  them in an endFunc 
****************************************************************************************/
...
#endif
	if ((pulseDelay < 0) || (pulseDuration < 1) || (nPulses < 0)){
...
/*constants for task modification signals - the higher order bits of doTask are reserved for flags, signalling that the task has been modified
and that the thread should wake up and change timing paramaters. 
Even with reserved bits, the number of tasks that can be requested can still be more than 16 million (2^24=16777216), which "should be enough
for anybody"
...
class pulsedThread{
	public:
		...
		*/
		pulsedThread (unsigned int , unsigned int , unsigned int , void *  , int (*)(void *, void * volatile &), void (*)(void * volatile), void (*)(void * volatile), int , int &);
		pulsedThread  (float , float , float , void *  , int (*)(void *, void * volatile &), void (*)(void * volatile), void (*)(void * volatile), int , int &);
		~pulsedThread();
		...
		void setCustomDataDelFunc  (void (*delFunc)( void * volatile)); // sets a function that will be run when a pulsedThread is about to be killed
		void * getCustomData (void); // returns a pointer to the custom data. 
		// setters for low, high, and end functions
		void setLowFunc (void (*loFunc)(void * volatile)); // sets the function that is called on low part of cycle
		void setHighFunc (void (*hiFunc)(void * volatile)); // sets the function that is called on high part of cycle
	...
	private:
		...
		void (*delCustomDataFunc)( void * volatile); 
};
